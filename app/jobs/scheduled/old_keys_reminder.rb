...
module Jobs
  class OldKeysReminder < ::Jobs::Scheduled
    ...
    def keys_list
      messages = old_site_settings_keys.map { |key| "#{key.name} - #{key.updated_at.to_date.to_formatted_s(:db)}" }
      old_api_keys.each_with_object(messages) { |key, array| array << "#{[key.description, key.user&.username, key.created_at.to_date.to_formatted_s(:db)].compact.join(" - ")}" }
      messages.join("\n")
    end
  end
end
